...
NAMESPACE="fcg-catalog"
IMAGE_NAME="${IMAGE_NAME:-fcg-catalog:local}"
...
docker build -t "$IMAGE_NAME" -f src/FCG.Catalog.WebApi/Dockerfile .
...
kubectl -n "$NAMESPACE" create secret generic fcg-catalog-local-secret \
...
echo "Applying FCG.Catalog"
kubectl apply -f "$MANIFEST_DIR/01-configmap.yaml" -f "$MANIFEST_DIR/02-app.yaml"

echo "Waiting for FCG.Catalog"
kubectl -n "$NAMESPACE" rollout status deployment/fcg-catalog --timeout=300s

echo ""
echo "FCG.Catalog is ready: http://localhost:5000"
