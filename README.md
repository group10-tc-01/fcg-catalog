...
|----------|-----------|:-----------:|---------|
| `ConnectionStrings:DefaultConnection` | Connection string do SQL Server | ✅ Sim | `Server=localhost;Database=fcg_catalog;User Id=sa;Password=SuaSenha;TrustServerCertificate=True;` |
...
| `Observability:OtlpEndpoint` | Endpoint OTLP para telemetria | ❌ Não | `https://otlp-gateway.grafana.net` |
| `Observability:OtlpAuthHeader` | Header de autenticação OTLP | ❌ Não | `Basic <token>` |
...
```bash
cd src/FCG.Catalog.WebApi

# Inicializar user-secrets (já feito se o .csproj contém UserSecretsId)
dotnet user-secrets init

# Configurar os secrets obrigatórios
dotnet user-secrets set "ConnectionStrings:DefaultConnection" "Server=127.0.0.1;Database=fcg_catalog;User Id=sa;Password=SuaSenhaForte123;TrustServerCertificate=True;"
...
# Secrets opcionais (Observabilidade - apenas se usar Grafana/OTLP)
dotnet user-secrets set "Observability:OtlpEndpoint" "https://..."
dotnet user-secrets set "Observability:OtlpAuthHeader" "Basic ..."
